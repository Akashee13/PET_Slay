...
SELECT p.id AS product_id, p.title, p.category, p.base_wholesale_price, p.moq, p.availability_status AS product_availability_status, p.is_new_arrival, p.media_cover_url,
...
func (r *PostgresRepository) Get(productID string) (*ProductDetail, error) {
	...
	err := r.db.QueryRow(getProductSQL, productID).Scan(
		&product.ID,
		&product.Title,
		&product.Category,
