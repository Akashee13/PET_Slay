...
docker build -f apps/admin/Dockerfile -t "us-docker.pkg.dev/${GCP_PROJECT_ID}/pet-slay/pet-slay-admin:${IMAGE_TAG}" .
...
gcloud run deploy "${CLOUD_RUN_SERVICE}" \
  ...
  --port 8080 \
  --set-env-vars "NEXT_PUBLIC_API_BASE_URL=${STAGE_API_BASE_URL}"
