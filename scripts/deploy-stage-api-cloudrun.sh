...
STAGE_BUYER_BEARER_TOKEN="${STAGE_BUYER_BEARER_TOKEN:-}"
...
gcloud run deploy "${CLOUD_RUN_SERVICE}" \
  ...
  --port 8080 \
  --set-env-vars "APP_ENV=stage,DATABASE_REQUIRED=true,DATABASE_URL=${STAGE_DATABASE_URL},SUPABASE_URL=${STAGE_SUPABASE_URL},SUPABASE_ANON_KEY=${STAGE_SUPABASE_ANON_KEY},SUPABASE_SERVICE_ROLE_KEY=${STAGE_SUPABASE_SERVICE_ROLE_KEY},STAGE_ADMIN_BEARER_TOKEN=${STAGE_ADMIN_BEARER_TOKEN},STAGE_BUYER_BEARER_TOKEN=${STAGE_BUYER_BEARER_TOKEN},FOUNDER_ADMIN_BEARER_TOKEN=${FOUNDER_ADMIN_BEARER_TOKEN}"
